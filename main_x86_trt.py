"""
    This file is a TensorRT version modified from `main_x86_trt.py`.
"""
import numpy as np
import os
import tracker_trt
...
if __name__ == '__main__':
    ...
    polygon_mask_blue_and_yellow = cv2.resize(polygon_mask_blue_and_yellow, custom_size)
    ...
    # 缩小尺寸，1920x1080->960x540
    color_polygons_image = cv2.resize(color_polygons_image, custom_size)
    ...
    draw_text_postion = (int(custom_size[0] * 0.01), int(custom_size[1] * 0.05))  # 绘制文本的位置
    ...
    writer = cv2.VideoWriter(os.path.join('./output', output_name), fourcc, fps, custom_size, True)
    ...
    while True:
        ...
        # 缩小尺寸，1920x1080->960x540
        im = cv2.resize(im, custom_size)  # 这里遇到一点问题，需要转为(800, 512)才能正常运行，1228800 = 3*800*512
        ...
        if len(bboxes) > 0:
            ...
            # 包含四角坐标，类别标签，跟踪ID
            list_bboxs = tracker_trt.update(bboxes, im)  # tracker_trt 是 deepsort

            # 画框（框+类别+ID）
            # 撞线检测点，(x1，y1)，y方向偏移比例 0.0~1.0
            output_image_frame = tracker_trt.draw_bboxes(im, list_bboxs, line_thickness=None, offset=offset)
            pass
        else:
            # 如果画面中 没有bbox 输出原始图像
